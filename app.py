...
car_models = st.multiselect(
    'Choose models of vehicle that you prefer',
    options=[str(model) for model in ['model']]
)

st.write('You selected:', car_models)
...
fig = px.scatter(filtered_data, x="model", y="price", size="pop", color="condition", hover_name="model", log_x=True, size_max=60)
st.plotly_chart(fig)
